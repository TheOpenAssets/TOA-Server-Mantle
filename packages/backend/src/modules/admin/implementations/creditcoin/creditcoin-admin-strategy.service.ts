...
@Injectable()
export class CreditCoinAdminStrategy implements IAdminDomainStrategy {
  private readonly logger = new Logger(CreditCoinAdminStrategy.name);
  ...
  async registerAsset(assetId: string): Promise<any> {
    ...
    if (asset) {
      await this.notificationService.create({
        ...
        header: 'Asset Registered On-Chain (Credit Coin)',
        detail: `Your asset ${asset.metadata.invoiceNumber} has been successfully registered on Credit Coin.`,
    ...
    return {
      success: true,
      message: 'Asset successfully registered on Credit Coin and database updated',
      assetId,
      status: AssetStatus.REGISTERED,
      transactionHash: txHash,
      // Explorer URL would be chain specific
    };
  }
  ...
  async deployToken(dto: DeployTokenDto): Promise<any> {
    ...
    if (asset) {
      await this.notificationService.create({
        userId: asset.originator,
        walletAddress: asset.originator,
        header: 'Token Deployment Complete (Credit Coin)',
        detail: `Your asset ${asset.metadata.invoiceNumber} has been tokenized on Credit Coin. Token address: ${result.tokenAddress}`,
    ...
    return {
      success: true,
      message: 'Token deployed successfully on Credit Coin and database updated',
      assetId: dto.assetId,
      status: AssetStatus.TOKENIZED,
  ...
  async listOnMarketplace(dto: ListOnMarketplaceDto): Promise<any> {
    ...
    await this.notificationService.create({
      userId: asset.originator,
      walletAddress: asset.originator,
      header: 'Asset Listed on Marketplace (Credit Coin)',
      detail: `Your asset ${asset.metadata.invoiceNumber} is now live on the Credit Coin marketplace.`,
    ...
    return {
      success: true,
      message: 'Token listed on Credit Coin marketplace',
      assetId: dto.assetId,
      transactionHash: txHash,
    };
  }
  ...
  async revokeAsset(assetId: string, reason: string): Promise<any> {
    ...
    return {
      success: true,
      message: 'Asset revoked on-chain (Credit Coin)',
      assetId,
      transactionHash: txHash,
    };
  }
  ...
  async approveMarketplace(assetId: string): Promise<any> {
    ...
    return {
      success: true,
      message: 'Marketplace approved to spend tokens on Credit Coin',
      assetId,
      transactionHash: txHash,
    };
  }
  ...
  async supplyYieldSettlement(settlementId: string): Promise<any> {
    ...
    this.logger.log(`[Credit Coin] Supplying yield settlement:`);
